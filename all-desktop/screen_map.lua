-- chunkname: @/var/folders/r9/xbxmw8n51957gv9ggzrytvf80000gp/T/com.ironhidegames.frontiers.windows.steam.ep3S4swo/all-desktop/screen_map.lua
		...
		if not level then
			...
			if victory.level_mode == GAME_MODE_CAMPAIGN then
				if not level[GAME_MODE_CAMPAIGN] then
					...
					if victory.level_idx < GS.last_level and not levels[victory.level_idx + 1] then
...
if IS_KR1 then
	...
	function HeroRoomViewKR1:initialize(size)
		...
		for i, d in ipairs(screen_map.hero_data) do
			local tpos = V.v((i - 1) % 4 * 75, math.floor((i - 1) / 4) * 77)
			local v2
		...
		if screen_map.user_data.heroes.selected then
			local hd = screen_map.hero_data[get_hero_index(screen_map.user_data.heroes.selected)]
